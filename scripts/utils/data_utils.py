...
#     data = copy.deepcopy(data)
    ...
    if quat_interpolate_method == 'slerp':
        ...
        for bone_idx in range(bones_amount):
            ...
            myo_timestamps = np.linspace(masked_vr_timestamps[0], masked_vr_timestamps[-1], num=for_interp)
...
def create_dataset(data_folder: Path,
    ...
    if use_preproc_data == True:
        ...
        for i, data in enumerate(exps_data): 
            if n_crop_idxs > 0:
                for key, value in data.items():
                    data[key] = value[:-n_crop_idxs] if key in input_keys else value[n_crop_idxs:]
        ...
        dataset = VRHandMYODataset(exps_data,
                               ...
                               transform = transform, 
                               down_sample_target=down_sample_target, # changed from python
    ...
    for one_exp_data_path in tqdm(all_paths):
        ...
        if data['data_myo'].shape[0] > 40_000:
            pass
            #data['data_myo'] = data['data_myo'][::2] # Changed by nikita
        ...
        for electrode in range(data['data_myo'].shape[1]):
            ...
            samp_freq = 250  # Sample frequency (Hz) # CHANGED FROM 250 original
        ...
        # here I (Vlad) normalize data based on separate signals.
        # I did it on the whole dataset, but that created vertical shifts (+- 0.2) in data
        maxx = np.nanmax(empty)
        minn = np.nanmin(empty)

        emg_min_max = (empty - minn) / (maxx - minn) # (0, 1)
        emg_min_max = 2 * emg_min_max - 1
        
        data_myo = emg_min_max - np.mean(emg_min_max, axis=0)
