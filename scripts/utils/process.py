...
def aggregate_batch_results(previous_aggregation: dict,
                            new_results: dict,
                            inplace: bool = True) -> dict:
    if inplace:
        next_aggregation = previous_aggregation
    else:
        next_aggregation = copy.copy(previous_aggregation)  # Deepcopy for more complex aggregations in future
...
def train_epoch(model,
                ...
                is_batch_scheduler: bool = False) -> dict:
    ...
    for step, data in batches_pbar:
        batch_results = train_batch(model=model,
                                    optimizer=optimizer,
                                    loss_func=loss_func,
                                    device=device,
                                    data=data)
        ...
        if not step:
            ...
            epoch_results = aggregate_batch_results(epoch_results, batch_results, inplace=False)
...
def val_epoch(model,
    ...
    for step, data in batches_pbar:
        batch_results = val_batch(model=model,
                                  loss_func=loss_func,
                                  device=device,
                                  data=data)
...
def visualize_val_moves(model, val_exps_data, epoch, device):
    ...
    for n, raw_data in enumerate(val_exps_data):
        ...
        wandb.log({f"visualization/{n}_move": [wandb.Video(gt_path, fps=new_fps), 
                                               wandb.Video(pred_path, fps=new_fps)]}, epoch)
        del hand_gt
        del hand_pred
