...
'''
INFO
We've used Myo version 1.5.1970.2
'''


def lsl_info_get(srate, name, type, n_channels):
    srate = srate  # Hz here
    name = name
    type = type
    n_channels = n_channels

    opts, args = getopt.getopt(sys.argv[1:], "hs:c:n:t:", longopts=["srate=", "channels=", "name=", "type"])
    for opt, arg in opts:
        if opt in ("-s", "--srate"):
            srate = float(arg)
    ...
    # first create a new stream info (here we set the name to mediapipe,
    # the content-type to video, 2 channels, 100 Hz, and float-valued data) The
...
class Myo(object):
    ...
    '''
    1. quat - quaternions, 4 values
    2. gyro - rotation in 3D
    3. acc - acceleration in 3D
    '''
    ...
    '''
    Sends classification of poses. Does not work here, though
    '''
    ...
    '''
    Indicates on which arm Myo is attached.
    '''
...
# m.add_imu_handler(quat, gyro, acc)
# m.add_pose_handler(lambda p: print('pose', p))
# m.add_arm_handler(lambda arm, xdir: print('arm', arm, 'xdir', xdir))
# m.sleep_mode(1)
# m.set_leds([0, 255, 0], [0, 255, 0])  # m.set_leds([128, 128, 255], [128, 128, 255])  # purple logo and bar LEDs
# m.vibrate(1)
...
def main():
    ...
    srate, outlet = lsl_info_get(srate=200, name='band', type='myo', n_channels=8)
    srate_gyro, outlet_gyro = lsl_info_get(srate=50, name='physics', type='quat', n_channels=4)

    showPS = False  # whether to show FPS of myo
    ...
    def proc_emg(emg, moving, times=[]):
        ...
        if showPS:
            ...
            if len(times) > 20:
                print('Samples per second:', (len(times) - 1) / (times[-1] - times[0]))
                times.pop(0)

    def proc_quat(quat, gyro, acc):
        outlet_gyro.push_sample(quat)
        # print(quat)
    # (11535, -232, -1130, -11578)
