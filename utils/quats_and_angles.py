...
with open(
    Path(__file__).expanduser().absolute().parent
    / "hand_model/anna_train_slice10_mcp_local_ess.pickle",
    "rb",
) as handle:
...
for finger_name in finger_names:
    with open(
        Path(__file__).expanduser().absolute().parent
        / "hand_model"
        / f"mcp_to_bending_plane/{finger_name}.pickle",
        "rb",
    ) as f:
...
finger_start_pose = {
    "thumb": normalize_vector(
        np.array([1.79773151e-04, -9.56848596e-05, 1.40103373e-04])
    ),
    "index": normalize_vector(
        np.array([4.79965970e-04, -3.64983564e-05, 1.17722357e-04])
    ),
    "middle": normalize_vector(
        np.array([4.78224132e-04, -1.26683160e-05, 8.61489718e-06])
    ),
    "ring": normalize_vector(
        np.array([4.43463102e-04, -3.26179546e-05, -8.73337173e-05])
    ),
    "pinky": normalize_vector(
        np.array([3.89465811e-04, -6.83795585e-05, -1.75300101e-04])
    ),
}
...
INDEX_TO_ANGLE_FULLNAME_MAPPING = {
    v: k for k, v in ANGLE_FULLNAME_TO_INDEX_MAPPING.items()
}
...
def angles_array_to_dict(angles_array: np.ndarray) -> dict:
    angles_dict = {
        finger_name: {"mcp": {}, "pip": {}, "dip": {}} for finger_name in finger_names
    }
...
def three_points_to_normal_vector(
...
def normal_vector_to_plane_equation(
    normal_vector: np.ndarray, point: np.ndarray
) -> np.ndarray:
...
def hand_quats_to_hand_angles(hand_quats: np.ndarray):
    ...
    wrist_quat_inv = (
        scipy.spatial.transform.Rotation.from_quat(wrist_quat).inv().as_quat()
    )
    hand_quats_wrist_normalized = np.stack(
        [multiply_quat(wrist_quat_inv, q) for q in hand_quats]
    )
    ...
    for f_name in finger_names:
        ...
        finger_angles = finger_quats_to_finger_angles(
            finger_quats_wrist_normalized, f_name
        )
...
def finger_quats_to_finger_angles(
    finger_quats_wrist_normalized: dict, finger_name: str
    ...
    e_x_local, e_y_local, e_z_local = LOCAL_ESS[
        finger_name
    ]  # local system of coordinates must be right
    mcp_vec_proj = mcp_vec - e_z_local * np.dot(mcp_vec, e_z_local) / np.dot(
        e_z_local, e_z_local
    )
...
def finger_angles_to_finger_quats(finger_angles: dict, finger_name: str):
    ...
    if finger_name == "thumb":
        ...
        mcp_proj_reproduced = (
            np.cos(mcp_theta) * e_x_local + np.sin(mcp_theta) * e_y_local
        ) * np.cos(mcp_fi)
        ...
        pip_rot = scipy.spatial.transform.Rotation.from_rotvec(
            pip_angle * normalize_vector(rot_vec)
        )
        pip_vec = pip_rot.apply(mcp_vec)

        dip_rot = scipy.spatial.transform.Rotation.from_rotvec(
            dip_angle * normalize_vector(rot_vec)
        )
        ...
        rotation_matrix = np.stack([vec_x, vec_y, vec_z]).T
        rot_from_matrix = scipy.spatial.transform.Rotation.from_matrix(rotation_matrix)
        mcp_quat = rot_from_matrix.as_quat()
        mcp_rot = scipy.spatial.transform.Rotation.from_rotvec(
            np.pi * normalize_vector(vec_x)
        )
        ...
        pip_rot = scipy.spatial.transform.Rotation.from_rotvec(
            np.pi * (170 / 180) * normalize_vector(vec_x)
        )
        ...
        finger_quats = {"mcp": mcp_quat, "pip": pip_quat, "dip": dip_quat}
