...
class HVATNetv3(nn.Module):
    def __init__(self,
                n_electrodes=8, n_channels_out=64,
                n_res_blocks=2, n_blocks_per_layer=2,
                n_filters=128, kernel_size=5,
                strides=(2, 2, 2, 2), dilation=2, 
                small_strides = (2, 2), 
                use_angles = False):
        ...
        self.denoiser = nn.Sequential(*[SimpleResBlock(n_filters, kernel_size) for i in range(n_res_blocks)])
        ...
        self.decoder_small = AdvancedDecoder(n_blocks_per_layer=n_blocks_per_layer,
                                             n_filters=n_filters, kernel_size=kernel_size,
                                            dilation=dilation, strides=small_strides[::-1])
        self.dropout4 = torch.nn.Dropout(p=0.5, inplace=False)


#         self.rnn = RNN(input_size= n_filters, hidden_size=n_filters, num_layers=5,output_size=n_filters)
        self.simple_pred_head = nn.Conv1d(n_filters, n_channels_out, kernel_size=1, padding='same')
    ...
    def forward(self, x):
        ...
        outputs_small[-1] = self.dropout3(self.mapper(outputs_small[-1]))
        emg_features = self.dropout4(self.decoder_small(outputs_small)[-1])
